...
- **IA**: Gemini externo, na mesma configuração já usada na VPS e nas funções atuais do sistema (`src/lib/imovel-ia.functions.ts`): chave lida de `integration_settings` (Configurações → Integrações, campo GEMINI_API_KEY) com fallback para a env `GEMINI_API_KEY`, chamando diretamente a API `generativelanguage.googleapis.com`. Modelo `gemini-2.5-flash` para mapeamento, normalização, status e vínculo de empreendimento; nos casos duvidosos de duplicidade usa-se um modelo mais forte da mesma conta. Respostas sempre em JSON estrito (responseMimeType application/json + schema), com parse defensivo em código. Processamento em lotes com limite de concorrência, respeitando rate limit (429) e barra de progresso.
