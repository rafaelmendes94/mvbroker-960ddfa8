    ...
    handlers: {
      GET: async () => {
        try {
          ...
          if (!supabase) {
            console.error("[feed/foto-video] env error:", envErr);
          ...
          if (imErr) {
            console.error("[feed/foto-video] DB error:", imErr.message);
          ...
          const candidatos = (imovData ?? []).filter((im: any) =>
          ...
          console.error("[feed/foto-video] unexpected:", e?.message || e);
