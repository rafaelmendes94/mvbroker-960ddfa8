...
import {
  Search, SlidersHorizontal, LayoutGrid, List, Bed, Bath, Car, Ruler, MapPin,
  X, Check, Eye, Download, ShoppingBag, Star, Pencil,
} from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/central")({
  head: () => ({ meta: [{ title: "Central de Imóveis — MV Broker" }] }),
  component: Central,
});
...
function Central() {
  ...
  function openDrawer(id: string) {
    setDrawerId(id);
    logImovel(id, "visualizado_central");
  }
  ...
  return (
    <>
        ...
        actions={
          <Button asChild variant="outline">
            <Link to="/exportacoes">
              ...
              Exportação <Badge className="ml-2">{exp.count}</Badge>
            </Link>
          </Button>
        }
      />
      ...
      <Card className="mb-4">
        <CardContent className="p-3 flex flex-col md:flex-row gap-2 items-stretch md:items-center">
          <div className="relative flex-1">
            ...
            <Input
              className="pl-9"
              placeholder="Busca rápida: código, título, cidade, bairro..."
              value={search}
          ...
          <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
            <SheetTrigger asChild>
              <Button variant="outline">
                ...
                Filtros {activeCount > 0 && <Badge className="ml-2">{activeCount}</Badge>}
              </Button>
            </SheetTrigger>
          ...
          {(activeCount > 0 || search) && (
            <Button variant="ghost" onClick={() => { setFilters(EMPTY); setSearch(""); }}>
              <X className="h-4 w-4 mr-1.5" /> Limpar
            </Button>
          )}
          <div className="flex gap-1 border rounded-md p-0.5">
            <Button size="sm" variant={view === "cards" ? "default" : "ghost"} onClick={() => setView("cards")}>
              <LayoutGrid className="h-4 w-4" />
            </Button>
            <Button size="sm" variant={view === "tabela" ? "default" : "ghost"} onClick={() => setView("tabela")}>
        ...
        <Card className="mb-4 border-primary/40">
          <CardContent className="p-3 flex items-center justify-between gap-2 flex-wrap">
            ...
            <div className="flex gap-2">
              <Button size="sm" onClick={addSelectedToExport}>
                <ShoppingBag className="h-4 w-4 mr-1.5" /> Adicionar à exportação
              </Button>
              <Button size="sm" variant="ghost" onClick={() => setSelected(new Set())}>
                <X className="h-4 w-4 mr-1.5" /> Limpar seleção
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      {loading ? (
        ...
        <Card><CardContent className="py-16 text-center text-sm text-muted-foreground">Nenhum imóvel encontrado com os filtros atuais.</CardContent></Card>
      ) : view === "cards" ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {pageItems.map((i) => (
            <ImovelCard
              ...
              inExport={exp.has(i.id)}
              canWrite={canWrite}
              onToggleSel={() => toggleSel(i.id)}
              onView={() => openDrawer(i.id)}
              onToggleExport={() => exp.toggle(i.id)}
            />
          ))}
        </div>
      ) : (
...
function ImovelCard({ imovel, selected, inExport, canWrite, onToggleSel, onView, onToggleExport }: any) {
  return (
      ...
      <CardContent className="p-4 space-y-2">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1 truncate">
              <MapPin className="h-3 w-3 shrink-0" />{[imovel.bairro, imovel.cidade].filter(Boolean).join(", ") || "—"}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-1.5 pt-2">
          ...
          <Button size="sm" variant={inExport ? "secondary" : "default"} className="flex-1" onClick={onToggleExport}>
            {inExport ? <><Check className="h-3.5 w-3.5 mr-1" />Na exportação</> : <>+ Exportação</>}
          </Button>
          {canWrite && (
            <Button size="icon" variant="ghost" asChild>
              <Link to="/imoveis/$id/editar" params={{ id: imovel.id }}><Pencil className="h-3.5 w-3.5" /></Link>
            </Button>
          )}
        </div>
      </CardContent>
